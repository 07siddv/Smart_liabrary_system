...
<html lang="en">
<head>
    <title>Issuing to: <%= name %></title>
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;600&display=swap" rel="stylesheet">
    <script src="https://unpkg.com/html5-qrcode" type="text/javascript"></script>
    
    <style>
        /* PURPLE THEME FOR ISSUE */
        ...
        .scan-book-box { background: white; padding: 20px; border-radius: 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); text-align: center; border-top: 5px solid #764ba2; }
        .scan-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; }
        
        /* Camera Box */
        #book-reader { width: 100%; border-radius: 10px; overflow: hidden; background: #eee; display: none; margin-bottom: 15px; }
...
<div class="container">
    <div class="profile-card">
        <img src="ImageLoader?img=<%= photo %>" class="profile-img" onerror="this.src='images/default.png'">
        
        <h2 style="margin: 10px 0;"><%= name %></h2>
        <p style="color: #666; font-size: 13px; margin: 0;"><%= officialId %></p>
        <div style="margin-top: 15px;">
            <span style="background: #eef2ff; color: #764ba2; padding: 5px 12px; border-radius: 20px; font-size: 12px; font-weight: bold;"><%= dept %></span>
        </div>
        <br>
    ...
    <div class="action-area">
        ...
        <div class="scan-book-box">
            <div class="scan-header">
                <h3 style="margin:0; color: #333;">📖 Issue Book</h3>
                
                <%-- MESSAGES FOR ISSUE --%>
                <% if("Success".equals(request.getParameter("msg"))) { %>
                    <span style="color: #28a745; font-weight: bold; background: #e8f5e9; padding: 5px 10px; border-radius: 5px; font-size: 13px;">✅ Book Issued!</span>
                <% } else if("OutOfStock".equals(request.getParameter("msg"))) { %>
                    <span style="color: #c62828; font-weight: bold; background: #ffebee; padding: 5px 10px; border-radius: 5px; font-size: 13px;">⚠️ Book Out of Stock</span>
                <% } else if("Error".equals(request.getParameter("msg"))) { %>
                    <span style="color: #c62828; font-weight: bold; background: #ffebee; padding: 5px 10px; border-radius: 5px; font-size: 13px;">⚠️ Issue Failed</span>
                <% } %>
            </div>

            <button id="startBtn" class="btn-start" onclick="startCamera()">📷 Scan Book QR</button>
            <button id="stopBtn" class="btn-start btn-stop" onclick="stopCamera()">🛑 Stop</button>

            <div id="book-reader" style="margin-top: 15px;"></div>
            
            <%-- FORM: POINTS TO ProcessIssueServlet --%>
            <form id="bookForm" action="ProcessIssueServlet" method="post" style="margin-top: 15px;">
                <input type="hidden" name="student_id" value="<%= officialId %>">
                <input type="text" id="bookIdInput" name="book_id" placeholder="Scan Book to Issue..." style="width: 100%; padding: 12px; border: 1px solid #ccc; border-radius: 8px; font-size: 14px; border: none;" autocomplete="off">
        ...
        <div class="history-box">
            ...
            <table>
                ...
                <tbody>
                    <%
                        // Fetch History
                        String query = "SELECT t.issue_date, t.due_date, t.status, b.title, b.author, b.unique_id " +
                    ...
                    <tr>
                        ...
                        <td style="color: #d9534f; font-weight: bold;"><%= histRs.getDate("due_date") %></td>
                        <td>
                            <% if("ISSUED".equals(histRs.getString("status"))) { %>
                    ...
                    <% } if(!hasData) { %> 
                        <tr><td colspan="5" style="text-align:center; color:#999; padding: 30px;">No books issued yet.</td></tr> 
                    <% } %>
                </tbody>
            </table>
        </div>
...
<script>
    ...
    function onBookScanSuccess(decodedText) {
        ...
        // Visual Feedback
        document.getElementById("book-reader").innerHTML = '<p style="padding:20px; font-weight:bold; color:#764ba2;">✅ Scanned! Issuing...</p>';
        document.getElementById("stopBtn").style.display = "none";

        let beep = new Audio('https://actions.google.com/sounds/v1/alarms/beep_short.ogg');
        beep.play();

        // Stop camera then submit
